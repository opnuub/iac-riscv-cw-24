...
```
This ensures any incorrectly fetched instructions are removed. In summary, forwarding (via ForwardAE and ForwardBE) bypasses data from the Memory or Writeback stages to the Execute stage, preventing unnecessary delays. Stalling (using stall and FlushE) temporarily halts the pipeline during load-use hazards to ensure data dependencies are resolved. Flushing (via FlushD and FlushE) removes incorrect instructions caused by control hazards such as branch mispredictions or jumps. These mechanisms ensure correct and efficient execution in the pipelined processor.
